...
def main():
    ...
    db_file = "merged_predictions.csv"
    ...
    if db.empty:
        ...
        selected_date = st.sidebar.selectbox(f"Past Predictions ", options=timestamps_list)
        ...
        if int(ws_buffer) - int(es_buffer) >= 5:
            ...
            # Include tabs here
            tab1, tab2 = st.tabs(["AlexNet", "ResNet"])

            with tab1:
                media = 'media'
                colname= '_alex'
                process_tab(selected_date, plage, events, media, nw_angle, sw_angle, es_buffer, ws_buffer, lower_threshold, upper_threshold, min_samples, distance_threshold, db, colname)
...
def process_tab(selected_date, plage, events, media, nw_angle, sw_angle, es_buffer, ws_buffer, lower_threshold, upper_threshold, min_samples, distance_threshold, db, colname):
    row = db.loc[db['obs_date'] == selected_date].sort_values('local_request_date', ascending=False).head(1)
    # print(row['local_request_date'].values[0])
    ...
    ar_names, ar_lat, ar_lon = retrieve_ar(row['local_request_date'].values[0], plage=plage, events=events, media=media)
    ...
    bounding_hulls_img, distances_df, score, ratio = post_hoc_explanation.explain(guidedgradcam, original, flares, nw_angle, sw_angle, es_buffer, ws_buffer, lower_threshold, upper_threshold, min_samples, distance_threshold, numpy=True)
    ...
    col2.image(normalize_image(np.load(guidedgradcam)), caption="Post hoc Explanation Map (Guided Grad-CAM)", use_column_width=True)
    ...
    flare_field = f"flare_probability{colname}"
    col1.markdown(f'<span style="color: blue; font-size: larger;"><b>Flare Probability (≥ M1.0) = {round((row[flare_field].values[0] * 100), 2)}%</b></span>', unsafe_allow_html=True)
