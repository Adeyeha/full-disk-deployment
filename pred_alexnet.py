...
fdp = FullDiskFlarePrediction(modelpath=PATH1,media_folder='alexnet',modeltype='customalexnet',rgb=False)
data = pd.read_csv('pred_dates.csv')

# Assuming 'data' is your DataFrame and 'fdp' is your predictor object
output_file = 'predictions_alexnet.txt'
...
with open(output_file, 'a') as file:
    for idx, row in data.iterrows():
        ...
        if pd.to_datetime(row['pred_date']) >= pd.to_datetime('2015-06-18'):
            ...
            prediction = fdp.predict(
                ...
                explanation_layer=fdp.model.features[10]
            )
