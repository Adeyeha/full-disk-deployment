...
def main():
    ...
    selected_date = st.sidebar.selectbox(
        "Past Predictions", options=timestamps_list
    )
    ...
    if int(ws_buffer) - int(es_buffer) >= 5:
        ...
        # col1.write(f"Flare Probability (≥ M1.0) = {round((row['flare_probabilty'].values[0] * 100),2) }%")
        # col1.markdown(f"**Flare Probability (≥ M1.0) = {round((row['flare_probabilty'].values[0] * 100),2)}%**")
        col1.markdown(f'<span style="color: blue; font-size: larger;"><b>Flare Probability (≥ M1.0) = {round((row["flare_probabilty"].values[0] * 100), 2)}%</b></span>', unsafe_allow_html=True)
